...
# Save model
nt_model <- rfe_results$fit
path_model<-"./output/model/"
saveRDS(rfe_results, str_c(path_model, "nt_rfe_results.rds"))
saveRDS(nt_model, str_c(path_model, "nt_model_fit.rds"))
#nt_model<-readRDS(str_c(path_model, "nt_model_fit.rds"))
...
#r_area_depth_grid<-subset(area_grid, names(nt_model$finalModel))
...
nt_map_es<-predict(model_grid,nt_model)

# Load mask
crop_mask<-rast("C:/Users/hounkpk1/Food_System/input/cropland_mask/mask_crop.tif")
nt_map_es_res<-resample(nt_map_es,crop_mask)
crp_nt_map_es<- nt_map_es_res %>% terra::mask(crop_mask, inverse=F)
writeRaster(crp_nt_map_es,"./output/Factor_Analysis/nt/nt_map_es.tif",overwrite=T)
plot(crp_nt_map_es)

#Load class of aridity and phosphorus
ar_p<-rast("./output/Factor_Analysis/nt/r_recod.tif")
es_ar_p<-c(crp_nt_map_es,ar_p)
...
# map with tmap
es_mag_maps<-tm_shape(crp_nt_map_es)+
...
# Saving the maps
png_es_nt_map<- es_mag_maps +
    tm_layout(legend.show=FALSE)

# save with tmap 
tmap_save(es_mag_maps,
          filename =  "./output/Factor_Analysis/nt/es_nt_map.pdf",
          width = 80,units='mm', dpi = 450)

# save with tmap 
tmap_save(png_es_nt_map,
          filename =  "./output/Factor_Analysis/nt/es_nt_map.png",
          width = 80,units='mm', dpi = 450)
...
ggsave(plt_map_es,filename = "./output/Factor_Analysis/nt/nt_boxplot.png",
...
# Save as PNG
png("./output/Factor_Analysis/nt/nt_shap_summary_plot.pdf", width = 800, height = 600, res = 120)
...
write_xlsx(shap_values,"./output/Factor_Analysis/nt/nt_grid_shap_vales.xlsx")
