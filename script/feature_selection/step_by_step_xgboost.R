...
# Save model
af_model <- rfe_results$fit
path_model<-"./output/model/"
saveRDS(rfe_results, str_c(path_model, "af_rfe_results.rds"))
saveRDS(af_model, str_c(path_model, "af_model_fit.rds"))
#af_model<-readRDS(str_c(path_model, "af_model_fit.rds"))
...
#r_area_depth_grid<-subset(area_grid, names(af_model$finalModel))
...
af_map_es<-predict(model_grid,af_model)

# Load mask
crop_mask<-rast("C:/Users/hounkpk1/Food_System/input/cropland_mask/mask_crop.tif")
af_map_es_res<-resample(af_map_es,crop_mask)
crp_af_map_es<- af_map_es_res %>% terra::mask(crop_mask, inverse=F)
writeRaster(crp_af_map_es,"./output/Factor_Analysis/af/af_map_es.tif",overwrite=T)
plot(crp_af_map_es)

#Load class of aridity and phosphorus
ar_p<-rast("./output/Factor_Analysis/af/r_recod.tif")
es_ar_p<-c(crp_af_map_es,ar_p)
...
# map with tmap
es_mag_maps<-tm_shape(crp_af_map_es)+
...
# Saving the maps
png_es_af_map<- es_mag_maps +
    tm_layout(legend.show=FALSE)

# save with tmap 
tmap_save(es_mag_maps,
          filename =  "./output/Factor_Analysis/af/es_af_map.pdf",
          width = 80,units='mm', dpi = 450)

# save with tmap 
tmap_save(png_es_af_map,
          filename =  "./output/Factor_Analysis/af/es_af_map.png",
          width = 80,units='mm', dpi = 450)
...
ggsave(plt_map_es,filename = "./output/Factor_Analysis/af/af_boxplot.png",
...
# Save as PNG
png("./output/Factor_Analysis/af/af_shap_summary_plot.pdf", width = 800, height = 600, res = 120)
...
write_xlsx(shap_values,"./output/Factor_Analysis/af/af_grid_shap_vales.xlsx")
