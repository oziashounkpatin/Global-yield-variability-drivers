...
p0<-gg_plot(df_mgt)+theme(axis.text.x=element_blank())
...
write_xlsx(df_bind1,"./output/mean_ci/overall_mean_ci.xlsx")
...
a<-p0+p1 + p2+ p3 +p4+p5+p6+p7+
  plot_layout(heights = c(0.40, 0.60,0.25,0.25,
                          0.25,0.25,0.25,0.30),
                          ncol =1)
  a
  
b<-p8+p9+p10+p11+p12+p13+p14+
  plot_layout(heights = c(0.50,0.50, 0.50,0.50,0.50,0.50,0.50,0.50),ncol =1)

b
ggsave(a,filename = "./output/graphs/all/myplot1.png",width = 10, height = 19, dpi = 300, units = "cm")
ggsave(b,filename = "./output/graphs/all/myplot2.png",width = 10, height = 19, dpi = 300, units = "cm")
