...
comb_gdd<-comb_gdd %>% mutate(crops=sub('....', '', cat), crops=gsub('.{1}$', '', crops))
...
"<0.8"
  ...
  ggplot(aes(x=cat1,y= mean,group=key, color=key)) +
  geom_point( size=2) +
    coord_flip()+
...
ggsave(plt_temperate,filename = "./Next_Article/output/graphs/recap/temperate/temperate_gdd.png",
       width = 30,height = 18.5,
       dpi = 300, units = "cm")
 
ggsave(plt_temperate,filename = "./Next_Article/output/graphs/recap/temperate/temperate_gdd.pdf",
       width = 45,height = 55,
       dpi = 300, units = "cm")
