...
df_tropical<-read_xlsx("./input/data/all_dis_cov3.xlsx",guess_max = 1000) %>%
    filter(key %in% c("AF"), !Crop_Group %in% c("Grass"),kg_clim %in% c("Tropical"))%>%
...
cat<-c("Cash crop","Maize","Other cereal","V_F_others")
...
cat<-c("0.20-0.50","0.50-0.65",">0.65")
...
#---@ for the wrb
li_wrb<-split(df_tropical$effectSize,df_tropical$wrb)
...
# Bind alls
df_all1<-bind_rows(all,crop, aridity,
                  pH,soc,p,bd,tex,
                  gdd_maize,
                  dem,slope,
                  wrb_bind,
...
df_all <- df_all1 %>% filter(!cat %in% c(">10","<10.9","coarse",
                                        #"GDD_maize1","GDD_maize2","GDD_maize3","GDD_maize4","GDD_maize5",
                                        "GDD_wheat1","GDD_wheat2","GDD_wheat3","GDD_wheat4","GDD_wheat5",
                                        "GDD_rice1","GDD_rice2","GDD_rice3","GDD_rice4","GDD_rice5",
                                        "GDD_soybean1","GDD_soybean2","GDD_soybean3","GDD_soybean4",
                                        "GDD_soybean5"))

# count_data<- count_data %>% filter(!Type_Cat %in% c("landform.17","landform.18","landform.19",
#                                                     "landform.20", "landform.21", "landform.22"))
# 
...
# final_df_new <-read_xlsx("./output/mean_ci/AF_clim/arng_AF_Tropical_mean_ci1.xlsx")
# final_df_new
#   
# # load for the graph
final_df1 <- final_df %>%
              slice(42,40,39,41,38,37:35,30:28,34,32,31,33,26,27,25:23,22:1) %>%
...
write_xlsx(df_bind2,"./output/mean_ci/AF_clim/AF_Tropical_mean_ci.xlsx")
write_xlsx(final_df,"./output/mean_ci/AF_clim/arng_AF_Tropical_mean_ci.xlsx")
write_xlsx(final_df1,"./output/mean_ci/AF_clim/arng_AF_Tropical_mean_ci_sliced.xlsx")

# #final_df1<- final_df1 %>% slice(1,)
# 
# # Plot with ggplot
# 
tr_theme2<-theme(
...
fig1<- final_df1 %>%
  ...
  # geom_vline(xintercept=c(1.5, 3.5,6.5,8.5,10.5,12.5,
  #                         14.5,16.5,17.5,21.5),
             # linewidth=0.25)+
...
# ggsave(fig1, filename = "./output/graphs/AF_clim/arid/AF_Arid_figure.png", width = 10, height = 15, dpi = 300, units = "cm")
# ggsave(fig1, filename = "./output/graphs/AF_clim/arid/AF_Arid_figure.pdf", width = 10, height = 15, dpi = 300, units = "cm")
# ggsave(hist_nt, filename = "./output/graphs/AF_Arid_hist.png", width = 7, height = 10, dpi = 300, units = "cm")
# 
# # #Get covariate data
...
#                                            "0.05-0.65",">0.65"))
# df_wrb<-final_df1 %>% select(cat,cat2,mean,ci_low,ci_high) %>%
#                          filter(cat %in% c("Acrisols","Albeluvisols","Alisols","Andosols","Arenosols",
#                                            "Calcisols","Cambisols","Chernozems","Ferralsols","Fluvisols",	
#                                            "Gleysols","Gypsisols","Histosols","Kastanozems","Leptosols",
#                                            "Lixisols","Luvisols","Phaeozems","Plinthosols","Podzols",
#                                            "Regosols","Solonetz","Vertisols"))
# 
# df_ldf<-final_df1 %>% select(cat,cat2,mean,ci_low,ci_high) %>%
#                          filter(cat %in% c("Mtn_sumt","Cliff_sl","Lwhi_mtn","Shills_dcsl","Lhgsl_steep",
#                                            "Lhgsl_mod","Mtn_vs","Mod_hills","Tfphi_dis","Tfphi_surf",	
#                                            "Val_sl","Tfplw_dis","Tfplw_surf","Hi_plain","Lw_plain"))
# 
# 
