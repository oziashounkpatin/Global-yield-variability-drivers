...
df_arid<-read_xlsx("./input/data/all_dis_cov3.xlsx",guess_max = 1000) %>%
    filter(key %in% c("AF"), !Crop_Group %in% c("Grass"),kg_clim %in% c("Arid"))%>%
...
cov<-c("no_cov")
fig<-c("overall")
all<-bind_cols(cov,cat,bt_all,fig) %>%
     dplyr::rename(cov=1,cat=2,fig=7)
...
fig<-cat
crop<-bind_cols(cov,cat,bt_crop,fig) %>%
     dplyr::rename(cov=1,cat=2,fig=7) 
crop
...
# final_df_new <-read_xlsx("./output/mean_ci/AF_clim/arng_AF_Arid_mean_ci1.xlsx")
# final_df_new
#   
# # load for the graph
final_df1 <- final_df %>%
              slice(33,31,30,32,29,26,24,23:22,21:20,19:18,17:16,15:13,12:1) %>%
              dplyr::mutate(ID = row_number()) %>%
              arrange(desc(ID))

# save the data and plots
write_xlsx(df_bind2,"./output/mean_ci/AF_clim/AF_Arid_mean_ci.xlsx")
write_xlsx(final_df,"./output/mean_ci/AF_clim/arng_AF_Arid_mean_ci.xlsx")
write_xlsx(final_df1,"./output/mean_ci/AF_clim/arng_AF_Arid_mean_ci_sliced.xlsx")
...
fig1<- final_df1 %>%
  ...
  #                         14.5,16.5,17.5,21.5),
             # linewidth=0.25)+
  scale_x_discrete(labels=final_df1$cov)+
  tr_theme2 +
  coord_flip()
