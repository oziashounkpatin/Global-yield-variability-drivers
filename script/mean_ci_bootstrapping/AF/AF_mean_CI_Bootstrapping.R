...
df_af<-read_xlsx("./input/data/all_dis_cov2_last.xlsx",guess_max = 1000) %>%
...
# draw plot 
hist_AF<-ggplot(df, aes(x = effectSize)) + 
    geom_histogram(aes(y =after_stat(density)),
                   #breaks = seq(-50, 50, by = 10), 
                   colour = "black", 
                   fill = "white") +
       stat_function(fun = dnorm, 
              args = list(mean = mean(df_af$effectSize), 
              sd = sd(df_af$effectSize)))+
    tr_theme1
...
#---@ overall
bt_all<-es_boot(df_af$effectSize)
...
#---@ for the managements
li_mgt<-split(df_af$effectSize,df_af$key)
...
#---@ for the crop groups
li_crop<-split(df_af$effectSize,df_af$Crop_Group)

bt_crop<-as.data.frame(t(map_df(li_crop, es_boot))) %>%
   rownames_to_column(var = "Type") %>%
   dplyr::rename(mean=2, ci_low=3,ci_high=4)

cat<-c("Cash crop","Cereal","Maize","V_F_others")
...
#---@ for the kg_clim
li_kg_clim<-split(df_af$effectSize,df_af$kg_clim)
...
li_aridity_class<-split(df_af$effectSize,df_af$aridity_class)
...
cat<-c("0.05-0.20","0.20-0.50","0.50-0.65",">0.65")
...
#---@ for the ph_class
li_ph_class<-split(df_af$effectSize,df_af$ph_class)
...
#---@ for the soc_class
li_soc_class<-split(df_af$effectSize,df_af$soc_class)
...
#---@ for Phosphorus
li_p_class<-split(df_af$effectSize,df_af$p_class)
...
#---@ for the bd_class
li_bd_class<-split(df_af$effectSize,df_af$bd_class)
...
li_texture_class<-split(df_af$effectSize,df_af$texture)
...
li_gdd_maize_class<-split(df_af$effectSize,df_af$gdd_maize_class)

summary(as.factor(df_af$gdd_maize_class))
...
cat<-c("GDD_maize2","GDD_maize4","GDD_maize5")
...
li_gdd_wheat_class<-split(df_af$effectSize,df_af$gdd_wheat_class)
...
li_gdd_rice_class<-split(df_af$effectSize,df_af$gdd_rice_class)

bt_gdd_rice_class<-as.data.frame(t(map_df(li_gdd_rice_class, es_boot))) %>%
   rownames_to_column(var = "Type") %>%
   dplyr::rename(mean=2, ci_low=3,ci_high=4)

cat<-c("GDD_rice2","GDD_rice4","GDD_rice5")
...
li_gdd_soybean_class<-split(df_af$effectSize,df_af$gdd_soybean_class)

bt_gdd_soybean_class<-as.data.frame(t(map_df(li_gdd_soybean_class, es_boot))) %>%
   rownames_to_column(var = "Type") %>%
   dplyr::rename(mean=2, ci_low=3,ci_high=4)

cat<-c("GDD_soybean2","GDD_soybean4","GDD_soybean5")
...
#---@ for the dem_class
li_dem_class<-split(df_af$effectSize,df_af$dem_class)
...
#---@ for the slope_class
li_slope_class<-split(df_af$effectSize,df_af$slope_class)
...
# Bind alls
df_all<-bind_rows(all,crop,clim,aridity,
                  pH,soc,p,bd,tex,
                  gdd_maize,#gdd_wheat,
...
write_xlsx(df_bind1,"./output/mean_ci/AF_mean_ci.xlsx")
...
fig1<- final_df %>%
  ...
  geom_vline(xintercept=c(3.5, 7.5,11.5,15.5,18.5,21.5,
                          24.5,27.5,30.5,31.5,
                          35.5),linewidth=0.25)+
...
ggsave(fig1, filename = "./output/graphs/all/AF_figure.png", width = 10, height = 15, dpi = 300, units = "cm")
ggsave(hist_AF, filename = "./output/graphs/hist/AF_hist.png", width = 7, height = 10, dpi = 300, units = "cm")

# save the data and plots
write_xlsx(df_bind1,"./output/mean_ci/AF_mean_ci2.xlsx")
...
write_xlsx(final_df,"./output/mean_ci/AF_clim/AF_mean_ci_plot_data.xlsx")
# write_xlsx(df_ph,"./output/mean_ci/AF_mean_ci2.xlsx")
# write_xlsx(df_soc,"./output/mean_ci/AF_mean_ci2.xlsx")
# write_xlsx(df_p,"./output/mean_ci/AF_mean_ci2.xlsx")
# write_xlsx(df_bd,"./output/mean_ci/AF_mean_ci2.xlsx")
# write_xlsx(df_tex,"./output/mean_ci/AF_mean_ci2.xlsx")
# write_xlsx(df_dem,"./output/mean_ci/AF_mean_ci2.xlsx")
# write_xlsx(df_slope,"./output/mean_ci/AF_mean_ci2.xlsx")
# write_xlsx(df_clim,"./output/mean_ci/AF_mean_ci2.xlsx")
# write_xlsx(df_ar,"./output/mean_ci/AF_mean_ci2.xlsx")
# write_xlsx(df_crops,"./output/mean_ci/AF_mean_ci2.xlsx")
# write_xlsx(df_crops,"./output/mean_ci/AF_mean_ci2.xlsx")
...
a_af<-p1 + p2+ p3 +p4+p5+p6+p7+
  plot_layout(heights = c(0.40, 0.25,0.25,0.25,
                          0.25,0.25,0.25,0.25,
                          0.50,0.50,0.50),
                          ncol =1)
a_af
...
all_af<-p1 + p2+ p3 +p4+p5+p6+p7+p8+p9+p10+p11+
...
ggsave(all_af,filename = "./output/graphs/AF_clim/overall/all_af.png",
       width = 10, height = 25, dpi = 300, units = "cm")

ggsave(a_af,filename = "./output/graphs/AF_clim/overall/AF_all1.png",
       width = 10, height = 19, dpi = 300, units = "cm")
ggsave(b_af,filename = "./output/graphs/AF_clim/overall/AF_all2.png",
       width = 10, height = 19, dpi = 300, units = "cm")
...
# Plot each covariate
a_p1<-p1+
  plot_layout(heights = c(0.40),
                          ncol =1)

a_p2<-p2+
  plot_layout(heights = c(0.15),
                          ncol =1)

a_p3<-p3+
  plot_layout(heights = c(0.25),
                          ncol =1)
...
ggsave(a_p1,filename = "./output/graphs/AF_clim/overall/af_a_p1.png",
       width = 10, height = 3, dpi = 300, units = "cm")

ggsave(a_p2,filename = "./output/graphs/AF_clim/overall/af_a_p2.png",
       width = 10, height = 2.2, dpi = 300, units = "cm")

ggsave(a_p3,filename = "./output/graphs/AF_clim/overall/af_a_p3.png",
       width = 10, height = 2.2, dpi = 300, units = "cm")

ggsave(a_p4,filename = "./output/graphs/AF_clim/overall/af_a_p4.png",
       width = 10, height = 2.2, dpi = 300, units = "cm")

ggsave(a_p5,filename = "./output/graphs/AF_clim/overall/af_a_p5.png",
       width = 10, height = 2.2, dpi = 300, units = "cm")

ggsave(a_p6,filename = "./output/graphs/AF_clim/overall/af_a_p6.png",
       width = 10, height = 2.2, dpi = 300, units = "cm")

ggsave(a_p7,filename = "./output/graphs/AF_clim/overall/af_a_p7.png",
       width = 10, height = 2.2, dpi = 300, units = "cm")

ggsave(a_p8,filename = "./output/graphs/AF_clim/overall/af_a_p8.png",
       width = 10, height = 3, dpi = 300, units = "cm")

ggsave(a_p9,filename = "./output/graphs/AF_clim/overall/af_a_p9.png",
       width = 10, height = 3, dpi = 300, units = "cm")

ggsave(a_p10,filename = "./output/graphs/AF_clim/overall/af_a_p10.png",
       width = 10, height = 3, dpi = 300, units = "cm")

ggsave(a_p11,filename = "./output/graphs/AF_clim/overall/af_a_p11.png",
       width = 10, height = 3, dpi = 300, units = "cm")
