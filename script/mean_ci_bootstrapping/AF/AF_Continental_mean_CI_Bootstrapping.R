...
df_continental<-read_xlsx("./input/data/all_dis_cov3.xlsx",guess_max = 1000) %>%
    filter(key %in% c("AF"), !Crop_Group %in% c("Grass"),kg_clim %in% c("Continental"))%>%
...
#df_all <- df_all %>% filter(!cat %in% c(">10","<10.9","coarse"))
...
final_df1 <- final_df %>%
              slice(24,23,22,18,21,19,20,17,16,15,14:1) %>%
              dplyr::mutate(ID = row_number()) %>%
              arrange(desc(ID))
final_df1 

# save the data and plots
write_xlsx(df_bind2,"./output/mean_ci/AF_clim/AF_Continental_mean_ci.xlsx")
write_xlsx(final_df,"./output/mean_ci/AF_clim/arng_AF_Continental_mean_ci.xlsx")
write_xlsx(final_df1,"./output/mean_ci/AF_clim/arng_AF_Continental_mean_ci_sliced.xlsx")
...
# ggsave(fig1, filename = "./output/graphs/AF_clim/AF_Continental_figure.png", width = 10, height = 15, dpi = 300, units = "cm")
# ggsave(hist_nt, filename = "./output/graphs/hist/AF_Continental_hist.png", width = 7, height = 10, dpi = 300, units = "cm")
