...
df_continental<-read_xlsx("./input/data/all_dis_cov3.xlsx",guess_max = 1000) %>%
    filter(key %in% c("OF"), !Crop_Group %in% c("Grass"),kg_clim %in% c("Continental")) %>%
...
cat<-c("Maize","Other cereal","Soybean","Veg&Fruit and others","Wheat")
cov<-c("crop","crop","crop","crop","crop")
...
#---@ for the soc_class
df_soc<-df %>% filter(!soc_class %in% c("1"))
...
cat<-c("GDD_maize1","GDD_maize2","GDD_maize3")
...
cat<-c("GDD_wheat2","GDD_wheat3","GDD_wheat4")
cov<-c("GDD_wheat","GDD_wheat","GDD_wheat")
...
#    dplyr::rename(mean=2, ci_low=3,ci_high=4)
# 
# cat<-c("GDD_rice1","GDD_rice2","GDD_rice3","GDD_rice4")
...
cat<-c("GDD_soybean1","GDD_soybean2","GDD_soybean3")
...
#---@ for the wrb
li_wrb<-split(df_continental$effectSize,df_continental$wrb)
...
df_all1<-bind_rows(all,crop, aridity,
                  pH,soc,p,bd,tex,
                  gdd_maize,gdd_soybean,
                  gdd_wheat,
                  dem,slope,wrb_bind,landform_bind
                  )

df_all <- df_all1 %>% filter(!cat %in% c("0.20-0.50"))
...
final_df1 <- final_df %>%
              slice(50,49,47,45,48,46,44:43,36:34,42:40,38:37,39,33:31,30:28,
                    27:1) %>%
...
write_xlsx(df_bind2,"./output/mean_ci/OF_clim/OF_continental_mean_ci.xlsx")
write_xlsx(final_df,"./output/mean_ci/OF_clim/arng_OF_continental_mean_ci.xlsx")
write_xlsx(final_df1,"./output/mean_ci/OF_clim/arng_OF_continental_mean_ci_sliced.xlsx")
...
fig1<- final_df %>%
  ...
  geom_vline(xintercept=c(3.5,6.5,9.5,11.5,15.5,18.5,
                          21.5,24.5,26.5,27.5, 30.5,
                          34.5),linewidth=0.25)+
...
# ggsave(fig1, filename = "./output/graphs/OF_clim/OF_Continental_figure.png", width = 10, height = 15, dpi = 300, units = "cm")
# ggsave(hist_nt, filename = "./output/graphs/OF_Continental_hist.png", width = 7, height = 10, dpi = 300, units = "cm")
# 
# # save the data and plots
# write_xlsx(df_bind2,"./output/mean_ci/OF_clim/OF_Continental_mean_ci.xlsx")
# write_xlsx(final_df,"./output/mean_ci/OF_clim/arng_OF_Continental_mean_ci.xlsx")
