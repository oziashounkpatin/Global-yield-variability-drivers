...
df_tropical<-read_xlsx("./input/data/all_dis_cov3.xlsx",guess_max = 1000) %>%
    filter(key %in% c("OF"), !Crop_Group %in% c("Grass"),kg_clim %in% c("Tropical"))%>%
...
li_crop$`Cash crop`<-NULL
li_crop$`Other cereal`<-NULL

bt_crop<-as.data.frame(t(map_df(li_crop, es_boot))) %>%
   rownames_to_column(var = "Type") %>%
   dplyr::rename(mean=2, ci_low=3,ci_high=4)

cat<-c("Maize","Rice","Soybean","V_F_others","Wheat")
...
final_df1 <- final_df %>%
              slice(34:31,29,30,28:27,21,20,26,23:22,25:24,19:18,17:16,
                    15:1) %>%
...
write_xlsx(df_bind2,"./output/mean_ci/OF_clim/OF_Tropical_mean_ci.xlsx")
write_xlsx(final_df,"./output/mean_ci/OF_clim/arng_OF_Tropical_mean_ci.xlsx")
write_xlsx(final_df1,"./output/mean_ci/OF_clim/arng_OF_Tropical_mean_ci_sliced.xlsx")
...
# 
# # save the data and plots
# write_xlsx(df_bind3,"./output/mean_ci/OF_clim/OF_Tropical_mean_ci.xlsx")
# write_xlsx(final_df,"./output/mean_ci/OF_clim/arng_OF_Tropical_mean_ci.xlsx")
# 
# # save the data and plots
