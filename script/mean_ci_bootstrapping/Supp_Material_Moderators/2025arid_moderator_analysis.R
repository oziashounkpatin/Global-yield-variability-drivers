...
# Get data per key
af1<-read_xlsx("./output/mean_ci/AF_clim/arng_AF_Arid_mean_ci_sliced.xlsx") %>% 
  dplyr::select(cov,cat,cat1,mean,ci_low,ci_high) %>%
  add_column(key="AF")

cc1<-read_xlsx("./output/mean_ci/CC_clim/arng_CC_Arid_mean_ci_sliced.xlsx") %>%
  dplyr::select(cov,cat,cat1,mean,ci_low,ci_high)%>%
  add_column(key="CC")

nt1<-read_xlsx("./output/mean_ci/NT_clim/arng_NT_Arid_mean_ci_sliced.xlsx") %>% 
  dplyr::select(cov,cat,cat1,mean,ci_low,ci_high)%>%
  add_column(key="NT")

of1<-read_xlsx("./output/mean_ci/OF_clim/arng_OF_Arid_mean_ci_sliced.xlsx") %>%
  dplyr::select(cov,cat,cat1,mean,ci_low,ci_high)%>%
  add_column(key="OF")
...
write_xlsx(comb_all,"./output/mean_ci/df_supp_mat1.xlsx")
# 
# min(comb_all1$mean, na.rm = TRUE);max(comb_all1$mean,na.rm = TRUE)
# min(comb_all2$mean, na.rm = TRUE);max(comb_all2$mean,na.rm = TRUE)
# min(comb_all3$mean, na.rm = TRUE);max(comb_all3$mean,na.rm = TRUE)
# min(comb_all4$mean, na.rm = TRUE);max(comb_all4$mean,na.rm = TRUE)
# 
# min(comb_all1$ci_low,na.rm = TRUE);max(comb_all1$ci_low,na.rm = TRUE)
# min(comb_all2$ci_low,na.rm = TRUE);max(comb_all2$ci_low,na.rm = TRUE)
# min(comb_all3$ci_low,na.rm = TRUE);max(comb_all3$ci_low,na.rm = TRUE)
# min(comb_all4$ci_low,na.rm = TRUE);max(comb_all4$ci_low,na.rm = TRUE)
# 
# 
# min(comb_all1$ci_high,na.rm = TRUE);max(comb_all1$ci_high,na.rm = TRUE)
# min(comb_all2$ci_high,na.rm = TRUE);max(comb_all2$ci_high,na.rm = TRUE)
# min(comb_all3$ci_high,na.rm = TRUE);max(comb_all3$ci_high,na.rm = TRUE)
# min(comb_all4$ci_high,na.rm = TRUE);max(comb_all4$ci_high,na.rm = TRUE)
...
"overall"
  ...
  geom_hline(yintercept = 0, linetype="dotted", 
   ...
   #facet_wrap(~mgt, ncol = 4)
# geom_vline(xintercept=c(14.5,23.5),linewidth=0.25)
...
ggsave(plt_arid,filename = "./output/graphs/supp_mat_1.png",
       width = 45,height = 75,
       dpi = 300, units = "cm") # 45
 
ggsave(plt_arid,filename = "./output/graphs/supp_mat_1.pdf",
       width = 45,height = 75,
       dpi = 300, units = "cm")
