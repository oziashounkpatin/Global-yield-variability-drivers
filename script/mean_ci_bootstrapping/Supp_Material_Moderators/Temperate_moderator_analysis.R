...
# Get data per key
af1<-read_xlsx("./Next_Article/output/mean_ci/AF_clim/arng_AF_temperate_mean_ci.xlsx") %>% 
...
cc1<-read_xlsx("./Next_Article/output/mean_ci/CC_clim/arng_CC_temperate_mean_ci.xlsx") %>%
  dplyr::select(cat,cat1,mean,ci_low,ci_high)%>%
  add_column(key="CC")

nt1<-read_xlsx("./Next_Article/output/mean_ci/NT_clim/arng_NT_temperate_mean_ci.xlsx") %>% 
  dplyr::select(cat,cat1,mean,ci_low,ci_high)%>%
  add_column(key="NT")

of1<-read_xlsx("./Next_Article/output/mean_ci/OF_clim/arng_OF_temperate_mean_ci.xlsx") %>%
  dplyr::select(cat,cat1,mean,ci_low,ci_high)%>%
  add_column(key="OF")
...
plt
plt_temperate<- plt + tr_theme1 
plt_temperate_legend<- plt + tr_theme2

plt_temperate
plt_temperate_legend

# save the maps
ggsave <- function(..., bg = 'white') ggplot2::ggsave(..., bg = bg)
ggsave(plt_temperate,filename = "./Next_Article/output/graphs/recap/temperate/temperate_all_RAP.png",
       width = 40,height = 50,
       dpi = 300, units = "cm")
 
ggsave(plt_temperate,filename = "./Next_Article/output/graphs/recap/temperate/temperate_all_RAP.pdf",
       width = 45,height = 55,
       dpi = 300, units = "cm")

ggsave(plt_temperate_legend,filename = "./Next_Article/output/graphs/recap/temperate/legend_temperate_all_RAP.pdf",
       width = 45,height = 55,
       dpi = 300, units = "cm")
