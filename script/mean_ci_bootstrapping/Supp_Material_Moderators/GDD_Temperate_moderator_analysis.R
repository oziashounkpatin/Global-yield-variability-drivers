...
# Get data per key
af1<-read_xlsx("./output/mean_ci/AF_clim/arng_AF_Temperate_mean_ci_sliced.xlsx") %>% 
  dplyr::select(cov,cat,cat1,mean,ci_low,ci_high) %>%
  add_column(key="AF")

cc1<-read_xlsx("./output/mean_ci/CC_clim/arng_CC_Temperate_mean_ci_sliced.xlsx") %>%
  dplyr::select(cov,cat,cat1,mean,ci_low,ci_high)%>%
  add_column(key="CC")

nt1<-read_xlsx("./output/mean_ci/NT_clim/arng_NT_Temperate_mean_ci_sliced.xlsx") %>% 
  dplyr::select(cov,cat,cat1,mean,ci_low,ci_high)%>%
  add_column(key="NT")

of1<-read_xlsx("./output/mean_ci/OF_clim/arng_OF_Temperate_mean_ci_sliced.xlsx") %>%
  dplyr::select(cov,cat,cat1,mean,ci_low,ci_high)%>%
  add_column(key="OF")
...
# bind all
gdd_Temperate<-bind_rows(comb_gdd_maize,comb_gdd_rice,
                    comb_gdd_soybean,comb_gdd_wheat)

gdd_Temperate <- gdd_Temperate %>% dplyr::mutate(cov=case_when(cov== "GDD_maize" ~ "maize",
                                                     cov== "GDD_rice" ~ "rice",
...
# Plot the GDD
plt1<-gdd_Temperate %>% dplyr::arrange(mean) %>%
...
plt1
plt_gdd_Temperate<- plt1 + tr_theme1 +theme(strip.text.x = element_blank())
plt_gdd_Temperate

# save the maps
ggsave <- function(..., bg = 'white') ggplot2::ggsave(..., bg = bg)
ggsave(plt_gdd_Temperate,filename = "./output/graphs/recap/Temperate/gdd_Temperate_all_RAP1.png",
       width = 35,height = 25,
       dpi = 300, units = "cm")
 
ggsave(plt_gdd_Temperate,filename = "./output/graphs/recap/Temperate/gdd_Temperate_all_RAP.pdf",
       width = 35,height = 25,
       dpi = 300, units = "cm")