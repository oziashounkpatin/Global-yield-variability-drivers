...
# bind all
gdd_arid<-bind_rows(comb_gdd_maize,comb_gdd_rice,
                    comb_gdd_soybean,comb_gdd_wheat)

gdd_arid <- gdd_arid %>% dplyr::mutate(cov=case_when(cov== "GDD_maize" ~ "maize",
                                                     cov== "GDD_rice" ~ "rice",
...
# Plot the GDD
plt1<-gdd_arid %>% dplyr::arrange(mean) %>%
...
plt1
plt_gdd_arid<- plt1 + tr_theme1 +theme(strip.text.x = element_blank())
plt_gdd_arid

# save the maps
ggsave <- function(..., bg = 'white') ggplot2::ggsave(..., bg = bg)
ggsave(plt_gdd_arid,filename = "./output/graphs/recap/arid/gdd_arid_all_RAP1.png",
       width = 35,height = 25,
       dpi = 300, units = "cm")
 
ggsave(plt_gdd_arid,filename = "./output/graphs/recap/arid/gdd_arid_all_RAP.pdf",
       width = 35,height = 25,
       dpi = 300, units = "cm")