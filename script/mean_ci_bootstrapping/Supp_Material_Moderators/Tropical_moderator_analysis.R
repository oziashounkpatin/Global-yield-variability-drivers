  ...
  # Get data per key
  af1<-read_xlsx("./Next_Article/output/mean_ci/AF_clim/arng_AF_tropical_mean_ci.xlsx") %>% 
  ...
  cc1<-read_xlsx("./Next_Article/output/mean_ci/CC_clim/arng_CC_tropical_mean_ci.xlsx") %>%
    dplyr::select(cat,cat1,mean,ci_low,ci_high)%>%
    add_column(key="CC")
  
  nt1<-read_xlsx("./Next_Article/output/mean_ci/NT_clim/arng_NT_tropical_mean_ci.xlsx") %>% 
    dplyr::select(cat,cat1,mean,ci_low,ci_high)%>%
    add_column(key="NT")
  
  of1<-read_xlsx("./Next_Article/output/mean_ci/OF_clim/arng_OF_tropical_mean_ci.xlsx") %>%
    dplyr::select(cat,cat1,mean,ci_low,ci_high)%>%
    add_column(key="OF")
  ...
  all_df3<-all_df2 %>% mutate(newClass = coalesce(cat,cat1)) %>%
                      dplyr::select(key,newClass,cat1,mean,ci_low,ci_high) %>%
                      rename(cat=2) %>% as.data.frame()
  
  all_df<-bind_cols(all_df1$cat,all_df3) %>% dplyr::rename(old_cat=1)
  
  af<-all_df %>% filter(key %in% c("AF")) %>% dplyr::select(old_cat:ci_high)
  cc<-all_df %>% filter(key %in% c("CC")) %>% dplyr::select(old_cat:ci_high)
  nt<-all_df %>% filter(key %in% c("NT")) %>% dplyr::select(old_cat:ci_high)
  of<-all_df %>% filter(key %in% c("OF")) %>% dplyr::select(old_cat:ci_high)
  
  # Create the empty moderator data frame
  df_crops0<-data.frame(old_cat=NA,cat = c("V_F_others","Cereal","Cash crop","Wheat",
                               "Soybean","Rice","Maize","overall"), 
                             cat1 = NA, mean = NA, ci_low=NA,ci_high=NA)
  
  df_aridity0<-data.frame(old_cat=NA,cat = c("<0.05","0.05-0.20","0.20-0.50","0.50-0.65",">0.65"), 
                             cat1 = NA, mean = NA, ci_low=NA,ci_high=NA)
  
  df_gdd0<-data.frame(old_cat=NA,cat = c("<0.8","0.8-2.7","2.7-4","4-6","6-10"), 
                             cat1 = NA, mean = NA, ci_low=NA,ci_high=NA)
  
  df_pH0<-data.frame(old_cat=NA,cat = c("alkaline","neutral","acidic"),
                             cat1 = NA, mean = NA, ci_low=NA,ci_high=NA)
  
  df_soc0<-data.frame(old_cat=NA,cat = c("<5","5-10",">10"),
                             cat1 = NA, mean = NA, ci_low=NA,ci_high=NA)
  
  df_p0<-data.frame(old_cat=NA,cat = c("<10.9","10.9-21.4",">21.4"),
                             cat1 = NA, mean = NA, ci_low=NA,ci_high=NA)
  
  df_bd0<-data.frame(old_cat=NA,cat = c("<1.20","1.20-1.47",">1.47"),
                             cat1 = NA, mean = NA, ci_low=NA,ci_high=NA)
  
  df_tex0<-data.frame(old_cat=NA,cat = c("fine","medium","coarse"),
                             cat1 = NA, mean = NA, ci_low=NA,ci_high=NA)
  
  df_dem0<-data.frame(old_cat=NA,cat = c("<250","250-1000",">1000"),
                             cat1 = NA, mean = NA, ci_low=NA,ci_high=NA)
  
  df_slope0<-data.frame(old_cat=NA,cat = c("<0.20","0.2-1","1-5","5-15",">15"),
                             cat1 = NA, mean = NA, ci_low=NA,ci_high=NA)
  ...
  ############## Combine all
  comb_all<-bind_rows(comb_crops,comb_gdd,comb_aridity,comb_pH,comb_soc,
  ...
  plt
  plt_tropical<- plt + tr_theme1 
  plt_tropical
  plt_tropical_legend<- plt + tr_theme2
  ...
  ggsave(plt_tropical,filename = "./Next_Article/output/graphs/recap/tropical/tropical_all_RAP.png",
         width = 40,height = 50,
         dpi = 300, units = "cm")
   
  ggsave(plt_tropical,filename = "./Next_Article/output/graphs/recap/tropical/tropical_all_RAP.pdf",
         width = 45,height = 55,
         dpi = 300, units = "cm")
  
  ggsave(plt_tropical_legend,filename = "./Next_Article/output/graphs/recap/tropical/legend_tropical_all_RAP.pdf",
         width = 45,height = 55,
         dpi = 300, units = "cm")
