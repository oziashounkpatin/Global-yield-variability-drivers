...
cat<-c("Maize","Other cereal","Rice","Soybean","Veg&Fruit and others","Wheat")
...
# df_bind$cat1<-paste0("'",df_bind$new_cat,"'")
# df_bind$cat2<-paste(df_bind$cat1,df_bind$cat1,sep="=")
# df_bind %>% arrange(desc(no))
...
final_df
final_df1 <- final_df %>%
              slice(73,72,70,69,67,71,68,63:61,66:64,55:53,58:56,60:59,52:50,
                    49:47,46:1)%>%
...
write_xlsx(df_bind2,"./output/mean_ci/NT_clim/NT_continentalmean_ci.xlsx")
write_xlsx(final_df,"./output/mean_ci/NT_clim/arng_NT_continentalmean_ci.xlsx")
write_xlsx(final_df1,"./output/mean_ci/NT_clim/arng_NT_continentalmean_ci_sliced.xlsx")
...
fig1<- final_df1 %>%
  ...
  # geom_vline(xintercept=c(3.5, 6.5,10.5,13.5,16.5,21.5,24.5,
  #                         27.5,30.5,33.5,35.5,38.5,44.5),linewidth=0.25)+
...
# ggsave(fig1, filename = "./output/graphs/NT_clim/NT_Continental_figure.png", width = 10, height = 15, dpi = 300, units = "cm")
# ggsave(hist_nt, filename = "./output/graphs/NT_Continental_hist.png", width = 7, height = 10, dpi = 300, units = "cm")
