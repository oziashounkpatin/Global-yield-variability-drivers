...
df_Continental<-read_xlsx("./input/data/all_dis_cov2.xlsx",guess_max = 1000) %>%
    filter(key %in% c("AF","CC","NT","OF"), !Crop_Group %in% c("Grass"),kg_clim %in% c("Continental"))#%>%
...
#---@ for management
soil_long_red<-soil_long_nt %>% filter(!ID %in% c("only weed_control"))
li_Continental_class<-split(soil_long_nt$value,soil_long_red$ID)

bt_Continental_class<-as.data.frame(t(map_df(li_Continental_class, es_boot))) %>%
   rownames_to_column(var = "ID") %>%
   dplyr::rename(mean=2, ci_low=3,ci_high=4)

df_Continental<-bind_rows(bt_all,bt_Continental_class) 
df_Continental$cat<-"Continental"

df_Continental$ID<-paste(df_Continental$cat,"&",df_Continental$ID)
df_Continental_arg<- df_Continental %>% slice(1,3,2,5,8,4,7,6)

# Compute percent increase
df_Continental_perc<-df_Continental_arg %>% 
...
#bind all
df_Continental_n<-bind_cols(count_n,df_Continental_perc)

df_Continental_n
  
# save the data and plots
write_xlsx(df_Continental_n,"./output/mean_ci/NT_clim/NT_Continental_management.xlsx")                 
