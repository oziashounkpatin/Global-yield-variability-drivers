...
df_Tropical<-read_xlsx("./input/data/all_dis_cov2.xlsx",guess_max = 1000) %>%
    filter(key %in% c("AF","CC","NT","OF"), !Crop_Group %in% c("Grass"),kg_clim %in% c("Tropical"))#%>%
...
#---@ for management
li_Tropical_class<-split(soil_long_nt$value,soil_long_nt$ID)

bt_Tropical_class<-as.data.frame(t(map_df(li_Tropical_class, es_boot))) %>%
   rownames_to_column(var = "ID") %>%
   dplyr::rename(mean=2, ci_low=3,ci_high=4)

df_Tropical<-bind_rows(bt_all,bt_Tropical_class) 
df_Tropical$cat<-"Tropical"

df_Tropical$ID<-paste(df_Tropical$cat,"&",df_Tropical$ID)
df_Tropical_arg<- df_Tropical %>% slice(1,3,2,5,4)

# Compute percent increase
df_Tropical_perc<-df_Tropical_arg %>% 
...
#bind all
df_Tropical_n<-bind_cols(count_n,df_Tropical_perc)

df_Tropical_n

# save the data and plots
write_xlsx(df_Tropical_n,"./output/mean_ci/NT_clim/NT_Tropical_management.xlsx")                 
