...
df_arid<-read_xlsx("./input/data/all_dis_cov2.xlsx",guess_max = 1000) %>%
...
#---@ for management
li_arid_class<-split(soil_long_nt$value,soil_long_nt$ID)

bt_arid_class<-as.data.frame(t(map_df(li_arid_class, es_boot))) %>%
   rownames_to_column(var = "ID") %>%
   dplyr::rename(mean=2, ci_low=3,ci_high=4)

df_arid<-bind_rows(bt_all,bt_arid_class) 
df_arid$cat<-"Arid"

df_arid$ID<-paste(df_arid$cat,"&",df_arid$ID)

df_arid_arg<- df_arid %>% slice(1,4,3,2,5,7,6)

# Compute percent increase
df_arid_perc<-df_arid_arg %>% 
     mutate(mean1=perc(mean), cil=perc(ci_low),cih=perc(ci_high))
    
df_arid_perc %>% filter(mean1>100)
# count all obs 
...
#bind all
df_arid_n<-bind_cols(count_n,df_arid_perc)


df_arid_n

# save the data and plots
write_xlsx(df_arid_n,"./output/mean_ci/NT_clim/NT_arid_management.xlsx")                 
df_arid_n

hist(df_arid_n$mean1)
