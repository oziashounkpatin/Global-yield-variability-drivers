...
df_arid<-read_xlsx("./input/data/all_dis_cov3.xlsx",guess_max = 1000) %>%
    filter(key %in% c("CC"), !Crop_Group %in% c("Grass"),kg_clim %in% c("Arid")) %>%
                    drop_na(Crop_Group) %>%
...
cat<-c("GDD_rice2","GDD_rice3","GDD_rice5")
...
final_df1 <- final_df %>%
              slice(40,39,38,37,36:34,26:24,33:31,28:27,30:29,23:21,20:19,18:17,16:1) %>%
              dplyr::mutate(ID = row_number()) %>%
              arrange(desc(ID))

# save the data and plots
write_xlsx(df_bind2,"./output/mean_ci/CC_clim/CC_Arid_mean_ci.xlsx")
write_xlsx(final_df,"./output/mean_ci/CC_clim/arng_CC_Arid_mean_ci.xlsx")
write_xlsx(final_df1,"./output/mean_ci/CC_clim/arng_CC_Arid_mean_ci_sliced.xlsx")
...
# 
# # save the data and plots
# write_xlsx(df_bind3,"./output/mean_ci/CC_clim/CC_Arid_mean_ci.xlsx")
# write_xlsx(final_df,"./output/mean_ci/CC_clim/arng_CC_Arid_mean_ci.xlsx")
# 
