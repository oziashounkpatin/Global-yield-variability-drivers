...
df_emperate<-read_xlsx("./input/data/all_dis_cov3.xlsx",guess_max = 1000) %>%
    filter(key %in% c("CC"), !Crop_Group %in% c("Grass"),kg_clim %in% c("Temperate")) %>%
                    drop_na(Crop_Group) %>%
...
df<-read_xlsx("./input/data/all_dis_cov2.xlsx",guess_max = 1000) %>%
    ...
    dplyr::select(effectSize,key,Crop_Group,N_input,soil_cover,weed_control,rotation,
                 kg_clim,aridity_class,ph_class,soc_class,
                  ...
                  gdd_soybean_class, dem_class,slope_class,wrb,landform) %>%
                  drop_na(Crop_Group) %>%
    dplyr::mutate(Crop_Group = ifelse(Crop_Group == "Veg&Fruit and others",
      "V_F_others",Crop_Group)) %>%
     mutate(ES=perc(effectSize)) %>%
...
cat<-c("Cash crop","Maize","Other cereal","Rice","V_F_others","Wheat")
cov<-c("crop","crop","crop","crop","crop","crop")
...
cat<-c("<0.05","0.05-0.20","0.20-0.50")
...
cat<-c("GDD_maize2","GDD_maize3","GDD_maize4")
...
cat<-c("GDD_wheat3","GDD_wheat4","GDD_wheat5")
...
cat<-c("GDD_soybean2","GDD_soybean3","GDD_soybean4")
...
#---@ for the wrb
li_wrb<-split(df_emperate$effectSize,df_emperate$wrb)
...
df_all1<-bind_rows(all,crop, aridity,
                  pH,soc,p,bd,tex,
                  gdd_maize,gdd_wheat,gdd_soybean,gdd_rice,
                  dem,slope,wrb_bind,landform_bind
                  )

df_all <- df_all1 %>% filter(!cat %in% c("<0.20"))
...
final_df1 <- final_df %>%
              slice(60,58,56,54,57,59,55,53:51,43:41,50:48,46:44,47,40:38,37,36,34:1) %>%
              dplyr::mutate(ID = row_number()) %>%
              arrange(desc(ID))
final_df1

# save the data and plots
write_xlsx(df_bind2,"./output/mean_ci/CC_clim/CC_Temperate_mean_ci.xlsx")
write_xlsx(final_df,"./output/mean_ci/CC_clim/arng_CC_Temperate_mean_ci.xlsx")
write_xlsx(final_df1,"./output/mean_ci/CC_clim/arng_CC_Temperate_mean_ci_sliced.xlsx")
...
fig1<- final_df1 %>%
  ...
  # geom_vline(xintercept=c(3.5, 5.5,8.5,10.5,12.5,15.5,18.5,20.5,
  #                         22.5,27.5),linewidth=0.25)+
...
# ggsave(fig1, filename = "./output/graphs/CC_clim/CC_Temperate_figure.png", width = 10, height = 15, dpi = 300, units = "cm")
# ggsave(hist_nt, filename = "./output/graphs/hist/CC_Temperate_hist.png", width = 7, height = 10, dpi = 300, units = "cm")
# 
# # save the data and plots
# write_xlsx(df_bind3,"./output/mean_ci/CC_clim/CC_Temperate_mean_ci.xlsx")
# write_xlsx(final_df,"./output/mean_ci/CC_clim/arng_CC_Temperate_mean_ci.xlsx")
# 
# 8# save the data and plots
