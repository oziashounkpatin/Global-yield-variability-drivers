...
df_continental<-read_xlsx("./input/data/all_dis_cov3.xlsx",guess_max = 1000) %>%
    filter(key %in% c("CC"), !Crop_Group %in% c("Grass"),kg_clim %in% c("Continental"))%>%
...
cat<-c("GDD_soybean2","GDD_soybean3")
...
df_all1<-bind_rows(all,crop, aridity,
                  pH,soc,p,bd,tex,
                  gdd_maize,gdd_rice,
                  gdd_soybean,gdd_wheat,
                  dem,slope,wrb_bind,landform_bind
                  )

df_all <- df_all1 %>% filter(!cat %in% c("neutral"))
...
final_df1 <- final_df %>%
              slice(56,55,53,52,54,51,49:47,41:40,46,44:42,45,
                    39:38,37:35,34:1)%>%
...
write_xlsx(df_bind2,"./output/mean_ci/CC_clim/CC_Continental_mean_ci.xlsx")
write_xlsx(final_df,"./output/mean_ci/CC_clim/arng_CC_Continental_mean_ci.xlsx")
write_xlsx(final_df1,"./output/mean_ci/CC_clim/arng_CC_Continental_mean_ci_sliced.xlsx")
...
fig1<- final_df1 %>%
  ...
  # geom_vline(xintercept=c(2.5,4.5,6.5,8.5,11.5,15.5,
  #                         18.5,20.5,22.5,25.5,26.5,27.5, 
  #                         32.5),linewidth=0.25)+
...
# ggsave(fig1, filename = "./output/graphs/CC_clim/CC_Continental_figure.png", width = 10, height = 15, dpi = 300, units = "cm")
# ggsave(hist_nt, filename = "./output/graphs/CC_Continental_hist.png", width = 7, height = 10, dpi = 300, units = "cm")
# 
# # save the data and plots
# write_xlsx(df_bind2,"./output/mean_ci/CC_clim/CC_Continental_mean_ci.xlsx")
# write_xlsx(final_df,"./output/mean_ci/CC_clim/arng_CC_Continental_mean_ci.xlsx")
