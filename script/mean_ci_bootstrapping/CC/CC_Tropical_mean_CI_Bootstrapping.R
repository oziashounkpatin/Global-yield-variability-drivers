...
df_tropical<-read_xlsx("./input/data/all_dis_cov3.xlsx",guess_max = 1000) %>%
    filter(key %in% c("CC"), !Crop_Group %in% c("Grass"),kg_clim %in% c("Tropical")) %>%
                    drop_na(Crop_Group) %>%
...
cat<-c("Maize","V_F_others","Wheat")
cov<-c("crop","crop","crop")
...
cat<-c("GDD_soybean2","GDD_soybean5")
...
final_df1 <- final_df %>%
              slice(48:45,44:42,34:32,40,41,37:35,39,38,31:29,28:26,25:1) %>%
...
write_xlsx(df_bind2,"./output/mean_ci/CC_clim/CC_Tropical_mean_ci.xlsx")
write_xlsx(final_df,"./output/mean_ci/CC_clim/arng_CC_Tropical_mean_ci.xlsx")
write_xlsx(final_df1,"./output/mean_ci/CC_clim/arng_CC_Tropical_mean_ci_sliced.xlsx")
...
# ggsave(fig1, filename = "./output/graphs/CC_clim/CC_Arid_figure.png", width = 10, height = 15, dpi = 300, units = "cm")
# ggsave(hist_nt, filename = "./output/graphs/hist/CC_Arid_hist.png", width = 7, height = 10, dpi = 300, units = "cm")
# 
# # save the data and plots
# write_xlsx(df_bind3,"./output/mean_ci/CC_clim/CC_Tropical_mean_ci.xlsx")
# write_xlsx(final_df,"./output/mean_ci/CC_clim/arng_CC_Tropical_mean_ci.xlsx")
# 
