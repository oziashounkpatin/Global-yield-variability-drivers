...
df_cc_crops<- cc %>% select(cat,mean,ci_low,ci_high) %>%
                         filter(cat %in% c("overall","Maize","Rice","Soybean",
                                           "Wheat","Other cereal","Cash crop",
                                           "V_F_others")) %>% as.data.frame() %>%
                                            slice(1,3,2,4,5,6,7,8)

df_nt_crops<- nt %>% select(cat,mean,ci_low,ci_high) %>%
                         filter(cat %in% c("overall","Maize","Rice","Soybean",
                                           "Wheat","Other cereal","Cash crop",
                                           "V_F_others")) %>% as.data.frame() %>%
                                            slice(1,3,2,4,5,6,7,8)
...
p23<-gg_plot(df_of_gdd_wheat)+theme(axis.text.y=element_blank())
...
ggsave(driv,filename = "./output/graphs/egu_Fig_3_GDD.png",
